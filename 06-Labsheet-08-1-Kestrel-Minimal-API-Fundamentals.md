...
2. กลับไปที่หน้าเบราว์เซอร์แล้วกดปุ่ม **Refresh (F5)** สังเกตว่าเกิดอะไรขึ้น และอธิบายสั้นๆ ว่าทำไมจึงเป็นเช่นนั้น
   - **คำตอบ** คือกด รีเฟส ไม่มีอะไรเกิดขึ้น
...
<img width="1917" height="960" alt="image" src="https://github.com/user-attachments/assets/0eab49a8-f15b-4b66-8675-192d0daa0ccf" />
...
ประโยชน์ของการใส่ /api/:จำแนกประเภทบริการ: แยกแยะระหว่างบริการส่งคืนข้อมูล (JSON API) ออกจากหน้าเว็บแสดงผล (HTML/Static Web Pages) อย่างเป็นระเบียบ  ง่ายต่อการกำหนดความปลอดภัย: สามารถเลือกตั้งค่า Security Rules, CORS, หรือ Authentication ให้ส่งผลเฉพาะกลุ่ม /api/* ได้ง่ายรองรับ API Versioning: ช่วยให้ต่อยอดเป็น /api/v1/ หรือ /api/v2/ ในอนาคตโดยไม่กระทบ URL หน้าเว็บเดิมปัญหาหากไม่ใส่:เกิดความสับสนของโครงสร้างระบบ และมีโอกาสเกิดการชนกันของเส้นทาง (Route Conflicts) ระหว่าง URL สำหรับ UI และ Data Endpointจัดการนโยบายความปลอดภัยและสิทธิ์การเข้าถึงแบบเฉพาะเจาะจงทำได้ยากขึ้น