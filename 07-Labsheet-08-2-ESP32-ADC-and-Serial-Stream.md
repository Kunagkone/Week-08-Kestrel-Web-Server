...
1. ใน ESP-IDF v6.x ฟังก์ชัน `adc_oneshot_read()` คืนค่าผลลัพธ์เป็นตัวเลขแบบใด และมีช่วงค่าต่ำสุด-สูงสุดเท่าใดเมื่อตั้งค่าเป็น `ADC_BITWIDTH_12`
   - **คำตอบ:** คืนค่าเป็นตัวเลขจำนวนเต็ม (Integer) แบบค่าดิบ (Raw Value) มีช่วงค่าตั้งแต่ 0 ถึง 4095 (รวม 4,096 ระดับ ตามความละเอียด 12-bit)
1. ทำไมเราจึงต้องกดปุ่ม `Ctrl + ]` เพื่อปิด **ESP-IDF Monitor** ก่อนที่เราจะรันโปรแกรมฝั่ง C# Kestrel ในใบงานถัดไป
   *(คำใบ้: หากโปรเซส idf.py monitor ยังเปิดค้างอยู่ จะเกิดอะไรขึ้นกับพอร์ต COM)*
   - **คำตอบ:** 
เพื่อปลดล็อกพอร์ตสื่อสารอนุกรม (COM Port) เนื่องจากพอร์ต COM สามารถถูกใช้งาน (Occupied) ได้เพียงทีละ 1 โปรเซสเท่านั้น หากเปิด idf.py monitor ค้างไว้ โปรแกรมฝั่ง C# Kestrel จะไม่สามารถเข้าถึงหรือเปิดอ่านข้อมูลจากพอร์ต COM เดียวกันได้
---
